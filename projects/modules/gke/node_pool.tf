...
resource "google_container_node_pool" "pools" {
  ...
  node_config {
    ...
    machine_type = lookup(each.value, "machine_type", "n2-standard-4") // Node Pool 的機器類型
    # machine_type    = lookup(each.value, "machine_type", "zones/asia-east1-a/N2/custom-4-5120")
  ...
  // 每個節點的最大pod數量
  max_pods_per_node = lookup(each.value, "max_pods_per_node", 32)
